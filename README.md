...
- чтение инженерных портов оборудования и сантехнических приборов: положение, направление потока, допустимые категории и занятость;
- создание настоящих трубных соединений через `IEngineeringObjectConnector` с проверкой портов, категории и стилей;
...
Базовые операции проверены на **Renga Standard 8.12 / API 2.46** и **Renga Standard 9.3 / API 2.50**. Инженерный коннектор требует **Renga 9.0 / API 2.47** или новее; живая проверка выполнена на Renga 9.3 / API 2.50.
...
| `renga_system_categories` | Стабильные имена и значения категорий инженерных систем | Нет |
| `renga_list_styles` | Стили из разрешённых коллекций Renga | Нет |
| `renga_get_ports` | Порты объекта, допустимые категории, положение и текущее подключение | Нет |
| `renga_create_object` | Создание одного объекта | Да |
| `renga_set_parameter` | Изменение одного параметра | Да |
| `renga_create_pipe_connection` | Трубное соединение между портами через инженерный коннектор | Да |

### Поддерживаемые типы создания

Текущий каталог включает уровни, стены, балки, колонны, перекрытия, пластины, кровли, помещения, элементы, оборудование, сантехнические приборы, светильники, электроустановочные изделия, двери, окна и проёмы.
...
Для трубной разводки сначала вызовите `renga_get_ports` для обоих объектов, затем получите `pipe` и `pipe_fitting` через `renga_list_styles` и выберите только совместимые стили. `renga_create_pipe_connection` принимает стабильные `UniqueId` конечных объектов, нулевые индексы портов, имя категории из `renga_system_categories` и параметры трассировки в миллиметрах. Порт существующей трассы задаётся индексом `-1`; соединение трасса–трасса не поддерживается Renga. После каждого вызова сервер сравнивает снимки модели до и после операции и возвращает фактически созданные трассы, трубы и фитинги.

Не перегружайте запрос списком фитингов «на всякий случай». В Renga 9.3 / API 2.50 несовместимый элемент списка может дать `Invalid fitting components`; если preview с проверенным стилем трубы падает именно так, один раз повторите его без предпочтений фитингов и обязательно проверьте фактически созданные объекты до commit.
...
Сервер дополнительно проверен на Renga Standard 9.3 / API 2.50 через настоящий stdio MCP-клиент: discovery возвращает все 14 инструментов, чтение и безопасные preview-вызовы проходят через JSON-RPC, а трубное соединение отдельно проверено в preview и commit с повторным чтением портов и созданных объектов.
...
- инженерный MCP пока создаёт только трубные соединения; воздуховоды и электрические цепи будут добавляться отдельными инструментами после такой же живой проверки;
- `GlobalOrigin` порта может быть `null`, если установленная COM-регистрация не маршалит структуру `Placement3D` для позднего связывания; категория, занятость и индекс порта при этом остаются доступны;
- в Renga 9.3 / API 2.50 запись `MepRouteSlope` через общий контейнер параметров может не изменить 3D-контур трассы; самотёчный уклон нужно проверять по координатам сегментов, а плоскую трассу исправлять редактированием маршрута в Renga;
...
- светильники размещаются на реальном потолочном хосте, а розетки — на выбранной стороне стены вне интервалов дверей, окон, мебели и мокрых приборов.
